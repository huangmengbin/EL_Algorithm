...
public:
	...
	bool static is_draw(int first, Map* map_point) {			//////这个子函数是，深度优先搜索，可是我不会呀。。。。。

		bool searched[200007] = {false};
		vector<int>a_deep_tree;
		a_deep_tree.push_back(first);

		while (a_deep_tree.size()!=1||map_point[a_deep_tree.back()].next_one.size()!=0)
		{
			while (map_point[a_deep_tree.back()].next_one.size()!=0) {

				searched[a_deep_tree.back()] = true;
								
				a_deep_tree.push_back(map_point[a_deep_tree.back()].next_one.back());

				if (searched[a_deep_tree.back()] == true) {
					return true;
				}

			}
			a_deep_tree.pop_back();
			searched[map_point[a_deep_tree.back()].next_one.back()] = false;
			map_point[a_deep_tree.back()].next_one.pop_back();
		}
...
int main() {
		...
		while (lose_condition.size() != 0) {
			...
			for (unsigned i = 0; i < win_condition.size(); i++, tree_ptr++)
			{
				...
				if (len == 0) {////////////////////////////////////////////////////////////////////////终于赢了,输出
					...
					if(output_result.size()!=2)output_result.push_back(first_point);
		...
		if (draw == true) {
			...
			big_tree.clear();
			if (Map::is_draw( first_point ,map_point )) {
				cout << "Draw" << endl;
			}
			else {
				cout << "Lose" << endl;
